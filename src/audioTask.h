...
// Je lock les deux ensemble pour qu'ils soient cohérents.
...
void audioSetup() {
    ...
    float preset[6] = { 0.0, 0.0, 0.0, 2.0, 2.0, 4.0 };

    audioMod.setCurve(preset);
...
void audioLoop() {
    ...
    if (result == ESP_OK && nbBytesRead > 0) {      
        ...
        // Audio samples are on 24 bits with this microphone, but the I2C interface was configured for 16 bit.
        ...
        if (state == State::AUDIO) {
            // Convert to stereo with proper clipping
            for (int i = 0; i < nbSamplesRead; i++) {
